...
def find_nii_ha_best_fit(lam_nii_ha, flam_nii_ha, ivar_nii_ha, sii_bestfit):
    ...
    if (('sii6716_out' not in sii_models)&('sii6731_out' not in sii_models)):
        ...
        ## Test if narrow Ha is within 30% of [SII]
        ...
        if ((per_diff <= -30)|(per_diff >= 30)|(nii_ha_bestfit['ha_n'].amplitude.value == 0)):
            ## If sigma (Ha) is not within 30% of [SII] -- used fixed Ha version
            nii_ha_bestfit, n_dof = nii_ha_fit.fixed_ha_one_component(lam_nii_ha, flam_nii_ha, ivar_nii_ha, \
                                                          sii_bestfit)    
    else:
        nii_ha_bestfit, n_dof = nii_ha_fit.fixed_ha_two_components(lam_nii_ha, flam_nii_ha, ivar_nii_ha, \
                                                       sii_bestfit)
